...
void Init() {
    ...
    for (register int i = 2; i <= N; i++) {
        ...
        for (register int j = 1; j <= ptot && i * pnum[j] <= N; j++) {
            isnp[i * pnum[j]] = 1;
...
int muSum(register uint n) {
    if (n <= N) return mu[n];
    if (Smu[n]) return Smu[n];
    register int ret = 0;
    for (register uint l = 2, r; l <= n; l = r + 1) r = n / (n / l), ret += (r - l + 1) * muSum(n / l);
    return Smu[n] = 1 - ret; 
}
ull phiSum(register uint n) {
    if (n <= N) return phi[n];
    if (Sphi[n]) return Sphi[n];
    register ull ret = 0;
    for (register uint l = 2, r; l <= n; l = r + 1) r = n / (n / l), ret += 1ull * (r - l + 1) * phiSum(n / l);
...
### BSGS
BSGS 用于在 $\Theta(\sqrt{p})$ 时间内求解 $a^x\equiv b \pmod p$。$p$ 是质数。
...
后两个要求 $f,g$ 为数论函数~~，但是说真的大家好像都是在用第一种~~。
...
然后考虑在 $\bmod 2$ 意义下的 Lucas 定理，就可以得到：$\displaystyle\binom{i}{j} \equiv 1 \pmod{2}$ 当且仅当 $i \operatorname{and} j = j$，其中 $\operatorname{and}$ 为按位与运算。cool。
### 网格图路径计数 - 翻折法
